"""Run summary and provenance record for a processed expedition."""
...
from .manifest import ExpeditionManifest
...
def summarise(
    ds: xr.Dataset,
    *,
    manifest: ExpeditionManifest | None = None,
    ...
    Combines expedition identity, extent, sampling statistics, per-bit QC
    ...
    manifest:
        Manifest the run was configured by. Supplies expedition identity;
        without it the name falls back to the ``expedition`` dataset attribute.
    platform:
        Project-level platform defaults, typically the ``platform`` block of
        ``project.yaml``. The manifest's ``expedition`` block overrides these
        key by key.
    artifacts:
    ...
    """
    expedition = dict(platform or {})
    # Nearest configuration wins: the manifest describes one expedition and so
    # refines the installation-wide defaults.
    expedition.update(dict(manifest.expedition) if manifest else {})
    expedition.setdefault("name", ds.attrs.get("expedition", "unknown"))
    root = Path.cwd()
    return {
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    ...
    lines = [f"# {expedition.get('name', 'Expedition')}", ""]
    lines += ["_Auto-generated by yacht-co2; do not edit by hand._", ""]

    lines += ["## Platform", ""]
    lines += _table(
        [[key, value] for key, value in expedition.items() if key != "name"],
        ["field", "value"],
    )
...
def write_report(summary: Mapping[str, Any], destination: str | Path) -> dict[str, Path]:
    ...
    logger.success("Wrote run report to {}", paths["report_markdown"])
    return paths
