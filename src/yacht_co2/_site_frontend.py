...
@media(max-width:460px){.masthead{display:grid}.filters-row{display:grid}.chart-controls,.line-controls{display:grid;grid-template-columns:1fr auto}.axis-field{grid-column:1/-1}.line-control{display:flex;grid-column:1/-1;align-items:center;gap:.4rem}.line-select{max-width:none;width:100%}.button.add-line{grid-column:1/-1}.section-heading{display:grid}.section-heading .button{width:100%}.section-actions{width:100%}}
...
const resetButton=q('#reset-view');if(resetButton)resetButton.onclick=()=>{
 ...
 mapVariable=S.variables[0];q('#map-variable').value=mapVariable;mapHasFit=false;
 q('#good').checked=true;
